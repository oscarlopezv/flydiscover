<?php
    ...
    $ch = curl_init('https://pay.payphonetodoesposible.com/api/button/Confirm');
   ...
   $data=json_decode($data,true);
    ...
    $msj=$data["message"];
    $msjs=$data['errors']; 
    $idp=$data['clientTransactionId']; 
    $det=explode("||",$data['optionalParameter3']);
    $detalle=$det[1]; 
    $cliente=$det[0];
   ...
   //echo "<a href='$url'>$url</a> ";
    ...
    if ($status=='Approved'){
        $query="Insert into ventas (idpaquete,detalle,cliente,telefono,mail,fecha_registro,monto,payphoneid)
                values ('$idp','$detalle','$cliente','$tel','$mail',now(),'$monto','$idpayphone')";
        mysql_query($query) ;
        $msj='Transaccíon realiazada con éxito';
    }
